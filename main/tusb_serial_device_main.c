...
#pragma region USB Queues and utils
/// @brief Incoming queue for messages coming from USB (host->device)
static QueueHandle_t usb_rx_queue;

/// @brief Incoming message structure
typedef struct
{
    uint8_t buf[CFG_TUD_VENDOR_RX_BUFSIZE + 1]; // Data buffer for USB messages (64bytes +1)
    size_t buf_len;                             // Number of bytes received
    uint8_t itf;                                // Index of vendor interface
} usb_rx_msg_t;

/// @brief Outgoing queue for messages going to the host (device->host)
static QueueHandle_t usb_tx_queue;

/// @brief Outgoing message structure
typedef struct
{
    uint8_t buf[CFG_TUD_VENDOR_TX_BUFSIZE + 1]; // Data buffer for the USB message (64bytes +1)
    size_t len;                                 // Number of bytes to send
    uint8_t source;                             // Optional source (?)
} usb_tx_message_t;

/// @brief Utility to queue outgoing bytes in the USB transfer
/// @param data Bytes to send
/// @param len Amount of bytes to send
esp_err_t app_send_to_usb(const uint8_t *data, size_t len)
{
    usb_tx_message_t msg;

    if (!usb_tx_queue)
    {
        ESP_LOGE(__func__, "USB TX Queue does not exist");
        return ESP_FAIL;
    }
    if (len == 0)
    {
        ESP_LOGE(__func__, "Data length cannot be null");
        return ESP_ERR_INVALID_ARG;
    }
    if (len > sizeof(msg.buf))
    {
        ESP_LOGW(__func__, "Data to send will need to be split in multiple messages");
    }

    size_t remaining = len;
    while (remaining > 0)
    {
        if (remaining > sizeof(msg.buf)) // If it doesn't fit in a single message
        {
            memcpy(msg.buf, data + len - remaining, sizeof(msg.buf));
...
#pragma region DCD and line control
...
static void pl2303_send_status(void)
{
    ...
    if (combined & 0x02)
        status_byte |= 0x80; /* indicate CTS */
    if (combined & 0x01)
        status_byte |= 0x01; /* indicate DCD */
...
// Declaration of cb defined in 'usbd.h'
...
void tud_vendor_rx_cb(uint8_t itf, uint8_t const *buffer, uint16_t bufsize)
{
    ...
    xQueueSend(usb_rx_queue, &tx_msg, 0);
...
void app_main(void)
{
    ...
    usb_rx_queue = xQueueCreate(5, sizeof(usb_rx_msg_t));
    assert(usb_rx_queue);
    usb_rx_msg_t msg;
    ...
    usb_tx_queue = xQueueCreate(8, sizeof(usb_tx_message_t));
